...
async function command(action,form){const data=Object.fromEntries(new FormData(form));data.action=action;if('gpu'in data)data.gpu=true;for(const k of ['events','seed']){if(data[k])data[k]=Number(data[k]);else delete data[k]}const r=await fetch('/api/control',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)});$('command').textContent=await r.text()}
